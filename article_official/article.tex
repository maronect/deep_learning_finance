...
These findings highlight an important insight: \textit{poor point-wise prediction accuracy does not necessarily prevent an useful portfolio optimization}. The relevant question is not whether models can accurately predict individual returns (they cannot, as evidenced by negative $R_2$), but whether they can provide better expected return estimates than historical means for the purpose of portfolio construction. The portfolio performance results (Section~\ref{sec:portfolio_performance}) suggest that Ridge Regression, despite its poor point-wise accuracy, does provide such value\citep{prado2016, prado2020}.
...
The use of maximum Sharpe ratio optimization ensures that differences in expected return estimates are reflected in portfolio weights, providing a fair comparison across models. The walk-forward methodology guarantees realistic evaluation without data leakage, while the blending mechanism ($\alpha=0.3$) successfully prevents extreme predictions while preserving model signals. Together, these choices create a robust framework for assessing the practical value of ML-enhanced expected returns in portfolio optimization.

\section{Conclusion and Future Work}

This study examined whether simple machine learning models-Linear Regression and MLP-can improve the expected return estimates used in Markowitz portfolio optimization. Although the models showed weak point-wise predictive accuracy, portfolios based on the learned expected returns performed better than the classical historical-mean baseline, with Linear Regression delivering the strongest risk-adjusted results. These findings suggest that even simple models can improve the portfolio construction, but they also reveal limitations: the analysis relied solely on lag-based features, assumed a constant risk-free rate and did not include transaction costs or alternative market regimes.

Future work could incorporate richer information sets—such as technical indicators and macroeconomic variables—and evaluate more advanced architectures like LSTMs or Transformers. It would also be valuable to test robustness across different periods and compare results with others optimization frameworks such as risk parity or Black-Litterman. Overall, the results indicate that machine learning can enhance portfolio optimization by improving expected return estimates, but more evidence is needed to determine when these improvements hold consistently.
